...
- Heat from chained accurate kills raises the score multiplier and sends more and deadlier pursuers (more spawns, higher caps, more gunslingers and wolves). A trigger pull that hits nothing breaks the chain and Heat then decays; taking damage clears current Heat. The HUD shows Heat level and multiplier, a chain-timer bar, and brief HEAT UP / CHAIN BROKEN / COOLING / HEAT LOST cues (with sounds for gaining and losing Heat); the run report shows peak Heat. The player blinks during the short invulnerability window after taking a hit.
